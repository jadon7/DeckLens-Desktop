...
| 一个肉眼单一的图片区域出现一堆控制框 | 裁切原图完整区域，合并成一张位图，并删除所有碎片 | 保留多个相邻、重叠、嵌套的图片碎片 |
...
| 页面背景、圆形、圆角矩形、卡片可由基础图形表达 | 默认加入 shape 重建清单 | 只因为它不是 icon 就跳过矢量化判断 |
...
| 插入 SVG/PNG icon | 宽高必须保持 1:1，按中心点缩放适配原区域 | 为了填满原框把 icon 横向或纵向拉伸 |
...
| 准备交付前 | Agent 自己运行最终 `inspect`，并按自检清单复核碎片、卡片、背景、icon、重复层 | 把结果交给用户肉眼兜底检查 |
...
- 整页背景、页面底色、页面边框、背景上的大面积纯色或简单渐变区域。PPT 背景本身就是基础图形候选，不能只作为位图保留。
- 圆形、椭圆、圆角矩形、普通矩形、图片占位框、图标底座、黑色圆底白色线性图标这类基础几何组合。
...
- 一个项目预览图、案例截图、作品缩略图、黑底图片区域、人物照片或产品图里出现超过 2 个相邻/重叠 `picture`，默认判定为同一复杂位图组，除非能说明每个碎片都有独立编辑语义。
- 碎片之间没有可独立编辑的语义，例如不是独立按钮、独立图标、独立文字、独立图表数据，而只是同一张复杂图里的局部像素。

这种情况不要尝试逐块修复，也不要把碎片保留下来。应从原始输入图裁切出完整语义区域，作为一张位图插回 PPT，并删除或隐藏所有对应碎片层。例如：一张黑底人物/白色布料照片被拆成十几块，最终 PPT 里应该只有一张该照片图片层；一排项目卡片顶部的项目截图每张也应该最多是一张图片层，而不是一堆控制框。
...
判断 SVG 或 PPT shape 时，不只检查 icon。页面背景、卡片、圆形、矩形、圆角矩形、线条、分割线、图标底座、项目卡片、KPI 卡片、普通按钮都属于基础图形候选。

默认用 PPT shape 重建：

- 白色、浅灰、黑色、纯色或简单透明度的页面背景。
- 普通卡片背景、卡片描边、弱阴影、圆角和内边距。
- 左侧指标区里的圆形图标底、圆形徽章、编号圆、步骤圆。
- 项目卡片、信息卡片、截图外框、图片占位框、内容容器。
- 横线、竖线、分割线、底部装饰线、简单色带。

默认用 SVG 或图标库重建：

- 圆形底里的电脑、用户、点赞、箭头、勾叉、文档、屏幕、社交媒体等符号。
- 简单线性 icon、面型 icon、导航 icon、状态 icon。

默认保留或合并为位图：

- 卡片内部的复杂截图、照片、3D 图、复杂插画、纹理图。
- 背景是纸张纹理、强噪声、复杂光影或无法用简单渐变表达的区域。

一个对象可以拆成“shape 容器 + 位图内容 + 文本/icon”。例如项目卡片顶部的复杂截图应合并成一张位图，卡片白底、描边、标题、说明、百分比和底部线条应分别用 shape/text 重建。
...
- 一个截图、照片、案例预览图或黑底图片区域被拆成 3 个以上 `picture`，且这些 `picture` 互相重叠、相邻或共享同一个外接矩形。
- PPT 里选中该区域时出现多层控制框，但原图里肉眼只有一个图片对象。
...
如果最终仍有大量小 picture，或者一个视觉对象还对应多个 picture 控制框，但没有逐项解释，结果只能称为“原始分层草稿”，不能称为最终交付。
...
13. 插入 icon 时必须保持 1:1 比例。若原图候选框不是正方形，用 `min(width, height)` 作为 icon 尺寸，按原候选框中心点居中放置；需要背景圆、背景方或胶囊时，把背景作为单独 shape 绘制，不要拉伸 icon 本体。
14. icon 自检必须检查最终宽高差异。除非原图就是明显非等比 logo，否则 SVG/PNG icon 的宽高差异不得超过 1%。发现被拉伸时，回到后处理修正尺寸和位置。
15. 插入替换元素时保持原图层顺序。替换元素要放在被删除位图层相同的 z-order 位置，避免遮挡关系变化。
16. 对复杂位图组，优先从原始输入图按完整语义区域裁切，不要尝试把 DeckLens 的碎片重新拼起来。原图裁切能保留边缘、纹理和抗锯齿，拼碎片通常会留下缝隙、毛边和重复像素。
17. 每新增一个 shape/SVG/icon/合并位图替换原图内容，都必须删除或隐藏对应源位图层。不要让“新对象 + 旧碎片”同时叠在一起。最终 `inspect` 里如果还能看到同一位置的旧 picture，必须继续清理。
18. 不要运行 `npm install`、不要从网络下载图标包、不要要求用户安装图标依赖。
...
7. 完成基础图形审计：页面背景、卡片、圆形、矩形、圆角矩形、分割线是否已判断为 shape；未重建的必须说明保留原因。
8. 完成 Agent 自检，不依赖用户发现问题。自检必须至少回答：
   - 最终每页是否还存在一个肉眼单一图片对象被拆成多张 `picture`。
   - 原图可见卡片、圆形、页面背景是否进入 shape 判断。
   - 原图 icon 候选是否执行了 `icons find` 和 `icons render`。
   - 最终 icon 是否保持 1:1 比例，非正方形候选框是否按中心点等比适配。
   - 新增 shape/SVG/icon/合并位图后，对应源图片层是否删除。
   - 最终剩余 picture 是否都能解释为复杂图片、截图、纹理、照片或保留位图。
9. 最终说明包含：
   - 使用的转换模式。
   - 每页替换为 PPT 原生形状的元素。
