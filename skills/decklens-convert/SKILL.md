...
9. Post-process the layered PPTX according to the pre-conversion vectorization plan.
10. Return the generated `.pptx` path and summarize the mode used, the vector targets replaced, and any targets intentionally left raster.
11. If the requested output already exists, ask before using `--overwrite`.
...
7. Resolve icons semantically before drawing. Prefer stable names such as `mail`, `phone`, `arrow-right`, `calendar`, `user`, `check`, or `globe`, then run `decklens icons find <name> --json` to get local bundled SVG paths. Avoid copying noisy segmented pixels as the source of truth for icon shape.
...
10. Keep icons as vector sources when the target PPT renderer supports them. Use official SVG icons or native PPT shapes where possible. If a local previewer renders embedded SVG icons as document placeholders or broken-image markers, do not ship that preview-broken deck as the only output. Either convert the chosen SVG icon set to transparent PNG at high resolution while preserving one consistent icon style, or rebuild the icons with native PPT shapes. Report this fallback clearly.
